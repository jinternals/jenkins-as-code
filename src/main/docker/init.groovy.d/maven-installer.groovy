...
mavenTools = [
        [
                'name'  : 'maven3',
                //'label' : 'maven3',
                'url'   : 'file:/var/tools/apache-maven-3.5.4-bin.tar.gz',
...
mavenTools.each { mavenTool ->

        println("Setting up : ${mavenTool.name} ")

        def describableList = new DescribableList<ToolProperty<?>, ToolPropertyDescriptor>()

        def installer = new ZipExtractionInstaller(
                mavenTool.label as String,
                mavenTool.url as String,
                mavenTool.subdir as String);
